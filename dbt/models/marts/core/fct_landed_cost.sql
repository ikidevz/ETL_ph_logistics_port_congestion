...
fx_filled AS (
    SELECT
        s.shipment_id,
        COALESCE(
            MAX(CASE WHEN f.rate_date <= COALESCE(s.eta::DATE, CURRENT_DATE()) THEN f.rate_date END),
            MIN(f.rate_date)
        ) AS matched_rate_date
    FROM shipments s
    LEFT JOIN fx f ON TRUE
...
SELECT
    ...
    -- VAT (12%) is applied on CIF + duties per Philippine customs law
    ROUND(((s.declared_value_usd + s.freight_usd + s.insurance_usd)
